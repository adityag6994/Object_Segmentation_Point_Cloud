...
void cloud_cb(const sensor_msgs::PointCloud2ConstPtr& input){
  ...
  //       std::cerr << "    " << cloud->points[i] << std::endl;
    ...
    std::cout << (cloud_cluster2->points.size()) << std::endl;
    
    std::vector<int> inliers_test;
    pcl::SampleConsensusModelCylinder<pcl::PointXYZ>::Ptr
    model_s(new pcl::SampleConsensusModelSphere<pcl::PointXYZ> (cloud_cluster2));
