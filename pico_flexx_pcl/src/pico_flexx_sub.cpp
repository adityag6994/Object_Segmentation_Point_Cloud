...
/***********model matching*************/
  ...
  extract_1.setInputCloud(cloud_filtered);
  extract_1.setIndices(inliers_cylinder);
  extract_1.setNegative (false);
  pcl::PointCloud<PointT>::Ptr cloud_cylinder (new pcl::PointCloud<PointT> ());
  extract_1.filter (*cloud_cylinder);
  ...
  // cout << *inliers_cylinder.size() << endl;
/***********Centroid*********************/
...
temp.data = centroid[2];
// temp = centroid[2];
ROS_INFO("%f",temp.data);
// for(size_t i=0; i< cloud_cylinder->points.size() ; i++){
//     centroid[0] += cloud_cylinder->points[i].x;
//     centroid[1] += cloud_cylinder->points[i].y;
//     centroid[2] += cloud_cylinder->points[i].z;
// }

// centroid[0] = centroid[0]  / cloud_cylinder->points.size();
// centroid[1] = centroid[1]  / cloud_cylinder->points.size();
// centroid[2] = centroid[2]  / cloud_cylinder->points.size();

// cout << cloud_cylinder->points[i].x << " . " << cloud_cylinder->points[i].y << " . " << cloud_cylinder->points[i].z  << endl;
// if(_flag%50 == 0){
...
/*output the point cloud as clusters*/	
	...
	pcl::toROSMsg (*cloud_cylinder, *clusters);
